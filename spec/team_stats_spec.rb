...
RSpec.describe TeamStats do
  ...
  xit 'test 1 * team will hash requested team info' do
    expected = {
      "team_id" => "18",
      "franchise_id" => "34",
  ...
  xit 'filter games_teams by team_id' do

    expect(team.game_teams_filtered('6').size).to eq 510
  end

  xit 'filter games by team id' do
  ...
  xit 'can caculate hash of opponent average win percentage' do
    expected = {"19"=>0.45,
       "52"=>0.4,
       "21"=>0.37,
       "20"=>0.37,
  ...
  xit 'can create a merged array of game_teams and games ' do

    expect(team.sorted_games_array('6').size).to eq 510
  end

  xit 'test 4* can calculate average_win_percentage' do

     expect(team.average_win_percentage('6')).to eq(0.49)
   end

   xit 'can hash a team games by season' do

     expect(team.hash_team_games_by_season('18').size).to eq(6)
   end

   xit 'test 2* can calculate a teams best_season' do

     expect(team.best_season("6")).to eq "20132014"
   end

   xit 'test 3* can calculate a teams worst_season' do

     expect(team.worst_season("6")).to eq "20142015"
   end


   xit 'test 5* can calculate the most goals scored for a team' do

     expect(team.most_goals_scored("18")).to eq 7
   end

   xit 'test 6* can calculate the least goals scored for a team' do
